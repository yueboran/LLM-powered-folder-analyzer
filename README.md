...
这个项目会扫描指定根目录下的文件夹大小，按从大到小排序；然后**只挑选根目录第一层**体积最大的前 N 个子文件夹（默认 10 个）交给大模型进行分析（不递归），输出清理建议与风险提示。

大模型分析输入包含：文件夹路径、大小、TopN 排名上下文、以及该文件夹的（尽量）完整子文件夹结构树。当结构树过大导致提示词超出硅基流动 `max_prompt_tokens` 限制时，程序会自动缩小结构树并重试；单个目录分析失败也不会中断整次 TopN 分析。
...
如果你已经执行了上面的可编辑安装，也可以：

```powershell
.venv\Scripts\python.exe -m folder_agent.cli --root "D:\YourFolder" --output ".\report\analysis_report.md"
```

调用硅基流动前建议清空无效代理（很多环境里 `HTTP_PROXY/HTTPS_PROXY` 会被软件写成无效地址导致连接被拒绝）：

```powershell
$env:HTTP_PROXY=""
$env:HTTPS_PROXY=""
$env:NO_PROXY="api.siliconflow.cn"
```
...
- 文件夹大小统计基于递归遍历，超大目录可能耗时较长。
- 对于无权限访问的目录，程序会记录并跳过。
- 报告为 Markdown 格式，便于后续继续整理或导出。
- 当前实现使用 Python 标准库加载 `.env` 和发送 HTTP 请求，不依赖额外三方包。
